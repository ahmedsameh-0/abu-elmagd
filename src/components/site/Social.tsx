...
export function Social() {
  return (
    <footer
      id="contact"
      className="relative overflow-hidden bg-[#090909] px-4 pb-8 pt-16 text-white sm:pt-24"
    >
      ...
      <div className="relative mx-auto max-w-6xl">
        <div className="flex items-start justify-between gap-8">
          ...
          <div className="flex gap-2">
            {platforms.map((platform) => {
              ...
              const className =
                "grid h-10 w-10 place-items-center overflow-hidden rounded-full border border-white/15 text-lg text-white/70";
              const icon =
                "image" in platform ? (
              ...
              return url ? (
                <a
                  ...
                  className={`${className} transition-colors hover:border-white/50`}
                >
                  {icon}
                </a>
              ) : (
                <span key={platform.key} aria-label={platform.label} className={className}>
                  {icon}
                </span>
              );
            })}
        ...
        <div className="mt-14 overflow-hidden border border-white/10 bg-white/[0.02] px-4 py-10 sm:px-10 sm:py-16">
          <div className="footer-wordmark select-none text-center text-[clamp(4rem,17vw,13rem)] font-semibold leading-[0.72] tracking-[-0.04em] text-white/[0.045]">
            Abu Elmagd
          </div>
        </div>
        <div className="mt-14 flex flex-col items-start justify-between gap-6 border-t border-white/10 pt-6 text-sm font-medium text-white/55 sm:flex-row sm:items-center">
          <nav className="flex flex-wrap gap-x-8 gap-y-3" aria-label="روابط التذييل">
            ...
            <a href="#branches" className="transition-colors hover:text-white">
              فروعنا
            </a>
            <Link to="/rate" className="transition-colors hover:text-white">
              تقييم الخدمة
            </Link>
